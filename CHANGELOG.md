...
### Fixed
- **Antigravity usage attributed to placeholder model ids** ([#68](https://github.com/juliantanx/aiusage/issues/68)) — a usage row's numeric model id took precedence over the readable model stored beside it, so every model whose id was missing from the parser's hard-coded table (1050, 1071, 1264, 1298, 1299, 1318, …) was imported as `antigravity-model-<id>` with `provider=unknown` and a $0 cost, and the list grows with every model Antigravity ships. The parser now names a generation from its readable metadata first — the selected model slug, the executor model (embedded in the generation row or in the `executor_metadata` table), the `MODEL_PLACEHOLDER_M<n>` label (which denotes id `1000 + n`) and the display label — then from ids that the same database pairs with a readable name elsewhere, then from the hard-coded table, and falls back to a placeholder only when nothing readable exists. A usage event whose id differs from its generation's (Antigravity's helper model) is no longer named after the generation. The hard-coded ids for Claude 4.5 Sonnet/Haiku now resolve to the priceable `claude-sonnet-4-5`/`claude-haiku-4-5`. Antigravity parsing is versioned in `watermark.json`: the upgrade re-imports every conversation database once, and since record ids do not depend on the model, the corrected model, provider and cost replace the old rows in place instead of duplicating them.
- **Gemini 3.x Pro usage priced at $0** ([#69](https://github.com/juliantanx/aiusage/issues/69)) — LiteLLM lists Gemini 3.1 Pro only as `gemini-3.1-pro-preview`, and prefix matching only helps a model name that is longer than the registry key, so records correctly identified as `gemini-3.1-pro` kept `costSource=unknown`. The pricing registry now seeds builtin aliases `gemini-3.1-pro`, `gemini-3.1-pro-high`, `gemini-3.1-pro-low` → `gemini-3.1-pro-preview` (likewise the `gemini-3-pro` family → `gemini-3-pro-preview` and `gemini-3-flash` → `gemini-3-flash-preview`) whenever the target price exists, on every database open and after each pricing sync, never overriding a user or synced alias; the site's pricing sync seeds the same aliases. Record model names are unchanged — Antigravity's effort-qualified variants keep their identity and are no longer collapsed — and *Recalculate pricing* (or the one-time Antigravity re-import) repairs existing $0 records.
