...
- **Multi-device sync lost and duplicated records** — two machines sharing one GitHub or S3 sync target could report different totals, a phantom `unknown` device could appear, and ids retired by a local rebuild or by an id-generation change lived on remotely and on every peer. Root causes and fixes:
  ...
  - *Fail-closed reconciliation*: the GitHub backend now reports a missing file only for `ENOENT` and throws on every other read or listing failure; a listing or read error aborts the sync before anything is pruned. A namespace whose files vanished between listing and reading, contain a malformed line, or do not match their manifest (an S3 rewrite in progress or interrupted — including a record moving between day files) is skipped for that sync instead of being treated as authoritative; `aiusage sync` reports skipped namespaces. A local database failure while applying a pull is never swallowed either: it aborts the sync before reconciliation, which itself runs in one transaction.
  ...
  - Migration v14 adds `sync_record_claims` and `sync_retired_wire_ids`; the old Antigravity/Trae ids already pushed to the cloud backend are retracted with tombstones, which other devices now honour on pull. Rows pulled before the upgrade from a namespace that had already disappeared are pruned automatically when the device has only ever used one sync target; otherwise `sync --repair` reports them as orphaned and removes them with `--apply`.
