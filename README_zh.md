...
- 如果 1.5.13 及更早版本产生了跨设备重复记录，可先用 `aiusage sync --repair` 检查，再决定是否执行清理。详见[同步修复指南](./docs/sync-repair.md)。 自 1.5.17 起，每台设备的命名空间都是该设备的权威快照，详见[同步模型说明](./docs/sync-namespaces.md)。
